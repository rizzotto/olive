...
export default function ExecuteSteps(props) {
    ...
    const images = steps.map(step => {
      ...
      if (step.media_format) {
        ...
        return storage
          ...
          .getDownloadURL()
          .catch(getPlaceholder(recipe.recipe_id));
      } else return getPlaceholder(recipe.recipe_id);
    });
    Promise.all(images).then(images => {
      setImages(images);
      setLoadingImages(false);
    });
  }, [recipe.recipe_id, steps, voiceEnabled]);
  ...
  const handleNextStep = () => {
    if (stepIndex + 1 == steps.length) {
      ...
      if (recipe.status === 'P' && (userDoc.role === 2 || userDoc.role == 0))
        path = 'evaluation';
      else path = 'recipe-finished';
      ...
      {steps && (
        <>
          ...
          <div className={classes.content}>
            {!loadingImages && (
              <img
                src={images[stepIndex] || getPlaceholder(recipe.recipe_id)}
